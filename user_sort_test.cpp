...
#define USE_FISSION_PARAMETERS 1
 ...
 public:
     ...
     void GiveNames();
     bool CheckNaIpromptGate (double);
     bool CheckNaIbgGate (double);

#if USE_FISSION_PARAMETERS>0
     bool CheckPPACpromptGate (double);
     bool CheckPPACbgGate (double);
 ...
 private:
    ...
    TimeGate NaITimeCut;
     ...
     double tPpac(double t, /*!<Uncorrected CACTUS time.*/
                 double Esi /*!Calibrated SiRi back energy in keV.*/);
...
//    const int max_e = 20000, max_de = 10000;
    ...
    //Don´t have to zoom like crazy everytime I make a particle spectrum :)
    const int max_e = 35000, max_de = 16000;
     ...
     // [a.u.], because it's timing bins, not ns
     ...
     for(int f=0; f<8; ++f ) {
         ...
         h_ex_r[f] = Spec(ioprintf("h_ex_f%d", f), ioprintf("E_{x} ring %d", f),
                          2000, -2000, 30000, "E_{x} [keV]");

         //h_ex_r[f]->SetLineColor(f+1);
     }

     h_ede = Spec("h_ede", "E+#DeltaE all detectors", 2000, 0, max_e, "E+#DeltaE [keV]");
     h_ex  = Spec("h_ex", "E_{x} all detectors", 2000, -2000, 30000, "E_{x} [keV]");
     h_ex_nofiss  = Spec("h_ex_nofiss", "E_{x} all detectors, veto for fission", 2000, -2000, 30000, "E_{x} [keV]");
...
double UserXY::tPpac(double t, double Esi)
{
    const double a = tppac_corr_esi [0] + tppac_corr_esi [1]/(Esi +tppac_corr_esi [2]) + tppac_corr_esi [3]*Esi;
    return t - a;
}
#endif /* USE_FISSION_PARAMETERS */
// ########################################################################
...
#if USE_FISSION_PARAMETERS>0
PPACTimeCut.lower_prompt = ppac_time_cuts[0];
PPACTimeCut.higher_prompt = ppac_time_cuts[1];
PPACTimeCut.lower_bg = ppac_time_cuts[2];
PPACTimeCut.higher_bg = ppac_time_cuts[3];
#endif /* USE_FISSION_PARAMETERS */
 }
 // ########################################################################
 // Check if a variable is in the prompt/bg gate of NaI/PPACs
bool UserXY::CheckNaIpromptGate(double doubletime)
{
bool NaIpromptGate = doubletime > NaITimeCut.lower_prompt && doubletime < NaITimeCut.higher_prompt;
return NaIpromptGate;
}
 // #########
bool UserXY::CheckNaIbgGate(double doubletime)
{
bool NaIbgGate = doubletime > NaITimeCut.lower_bg && doubletime < NaITimeCut.higher_bg;
return NaIbgGate;
}
 // #########
#if USE_FISSION_PARAMETERS>0
 bool UserXY::CheckPPACpromptGate(double doubletime)
    {
    bool PPACpromptGate = doubletime > PPACTimeCut.lower_prompt && doubletime < PPACTimeCut.higher_prompt;
    return PPACpromptGate;
    }
     // #########
 bool UserXY::CheckPPACbgGate(double doubletime)
    {
    bool PPACbgGate = doubletime > PPACTimeCut.lower_bg && doubletime < PPACTimeCut.higher_bg;
    return PPACbgGate;
    }
 #endif /* USE_FISSION_PARAMETERS */
// ########################################################################
...
//****************************************************************************************************
  ...
  //      std::cout << id << std::endl;
       const double na_e = calib( (int)event.na[i].adc, gain_na[id], shift_na[id] );
